# Stem Automail Reconfiguration SVG

Status: third rendered schematic view, 2026-05-17.

This SVG is the rendered view of the structured stem automail reconfiguration
trace. It is not a separate design source. The authoritative artifact remains
`schematics/stem_automail_reconfiguration_trace.json`, and the checked-in SVG
must match `autarkic_systems/schematic_svg.py` renderer output exactly.

The rendered artifact lives in
`schematics/stem_automail_reconfiguration_trace.svg`.

## Render Boundary

The SVG shows:

- the triangular RLEM/Universal Cell node;
- north, east, and west ports from the structured trace;
- stem role before the transition and processor role after it;
- before memory `right` and after memory `left`;
- automail before `pl` and automail after `_`;
- the `step_stem_cell` transition function and automail flow;
- the four interpretive layer IDs from the shared schematic schema.

The SVG does not claim full PRC stem buffering, dynamic GELC
reconfiguration, or physical circulator verification. It renders the
executable automail subset from ADR-0019.

## Verification

Run:

```sh
python -m unittest tests.test_stem_automail_svg
```

The test parses the SVG as XML, checks its trace metadata, port and layer data
attributes, confirms the stem reconfiguration and automail details are visible,
and verifies that the committed SVG exactly matches renderer output for the
current JSON trace.

## Regeneration

If the JSON trace changes, regenerate the SVG through `render_schematic_svg()`
and keep the test output green. Do not edit the SVG by hand unless the renderer
is updated in the same ADR.
