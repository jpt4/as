...
- `autarkic_systems/github_submission.py` renders local git evidence for the
  ...
  `HEAD`, a fork `main` browser URL, and an origin `main` browser URL,
  normalizing common HTTPS and SSH GitHub remotes to web URLs.
...
- `python -m autarkic_systems.github_submission` emits a local text/JSON
  ...
  URL, the upstream origin `main` browser URL, how far local `HEAD` is ahead
  of upstream `origin/main`, and where upstream submission notes are tracked.
  The browser URLs are normalized from common GitHub HTTPS and SSH remote
  forms. It also reports local
