...
from autarkic_systems import fixed_point_substitution_representability_proof_target
from autarkic_systems.fixed_point_substitution_representability_proof_target import (
    REQUIRED_MISSING_PROOF_ARTIFACTS,
    REQUIRED_NON_CLAIMS,
    load_fixed_point_substitution_representability_proof_target,
    validate_fixed_point_substitution_representability_proof_target,
)


TARGET = Path("claims/fixed_point_substitution_representability_proof_target.json")
CERTIFICATE = Path(
...
class FixedPointSubstitutionRepresentabilityProofTargetTests(unittest.TestCase):
    def setUp(self):
        self.target = (
            load_fixed_point_substitution_representability_proof_target(
                TARGET,
    ...
    def test_checked_in_manifest_names_proof_target_inputs(self):
        ...
        self.assertEqual(
            ...
            "as-fixed-point-substitution-representability-proof-target-v1",
        )
        self.assertEqual(
            self.target.substitution_representability_certificate_path,
            str(CERTIFICATE),
        )
        self.assertEqual(
            self.target.substitution_representability_readiness_path,
            str(READINESS),
        )
        self.assertEqual(
            self.target.bridge_predecessor_readiness_coverage_path,
            str(COVERAGE),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.target.expected_covered_predecessor_case_kinds,
            (
                "diagonal-instance-closure",
                "substitution-graph-correctness-proof",
            ),
        )
        self.assertEqual(self.target.expected_support_surface_count, 5)
        self.assertEqual(self.target.expected_witness_output_code_length, 296)
        self.assertEqual(
    ...
    def test_checked_in_manifest_validates_proof_target(self):
        ...
        self.assertEqual(report.covered_predecessor_case_kinds, self.target.expected_covered_predecessor_case_kinds)
        self.assertEqual(report.missing_certificate_predecessor_count, 0)
        self.assertEqual(report.support_surface_count, 5)
        self.assertEqual(report.witness_output_code_length, 296)
    ...
    def test_json_payload_exposes_blocked_proof_target(self):
        ...
        payload = (
            ...
            .fixed_point_substitution_representability_proof_target_payload(report)
        )

        self.assertTrue(payload["accepted"])
        self.assertEqual(payload["failed_subjects"], [])
        self.assertEqual(payload["case_kind"], "substitution-representability-proof")
        ...
        self.assertEqual(
            payload["covered_predecessor_case_kinds"],
            [
                "diagonal-instance-closure",
                "substitution-graph-correctness-proof",
            ],
        )
        self.assertEqual(payload["missing_certificate_predecessor_count"], 0)
        self.assertEqual(payload["support_surface_count"], 5)
        self.assertEqual(payload["witness_output_code_length"], 296)
    ...
    def test_text_report_exposes_target_without_promotion(self):
        ...
        self.assertIn(
            "Fixed-point substitution representability proof target: accepted",
            text,
        )
        ...
        self.assertIn("Missing proof artifacts: formal substitution representability derivation", text)
    ...
    def test_stale_missing_proof_artifacts_are_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            target = load_fixed_point_substitution_representability_proof_target(path)
        ...
        self.assertIn(
            "fixed-point-substitution-representability-proof-target-artifacts",
    ...
    def test_stale_support_surface_count_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "target.json"
            data = json.loads(TARGET.read_text(encoding="utf-8"))
            data["expected_support_surface_count"] = 4
            path.write_text(json.dumps(data), encoding="utf-8")
            target = load_fixed_point_substitution_representability_proof_target(path)
        ...
        self.assertFalse(report.accepted)
        self.assertIn(
            "fixed-point-substitution-representability-proof-target-support",
            report.failed_subjects,
        )
        self.assertTrue(
            any("support surface count mismatch" in result.detail for result in report.results)
        )

    def test_stale_witness_length_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "target.json"
            data = json.loads(TARGET.read_text(encoding="utf-8"))
            data["expected_witness_output_code_length"] = 295
            path.write_text(json.dumps(data), encoding="utf-8")
            target = load_fixed_point_substitution_representability_proof_target(path)
        ...
        self.assertFalse(report.accepted)
        self.assertIn(
            "fixed-point-substitution-representability-proof-target-witness",
            report.failed_subjects,
        )
        self.assertTrue(
            any("witness output length mismatch" in result.detail for result in report.results)
        )
    ...
    def test_missing_dependency_is_structured_rejection(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "target.json"
            data = json.loads(TARGET.read_text(encoding="utf-8"))
            data["substitution_representability_readiness_path"] = (
                "claims/missing_substitution_representability_readiness.json"
            )
            path.write_text(json.dumps(data), encoding="utf-8")
            target = load_fixed_point_substitution_representability_proof_target(path)
        ...
        self.assertFalse(report.accepted)
        self.assertIn(
            "fixed-point-substitution-representability-proof-target-dependencies",
            report.failed_subjects,
        )
    ...
    def test_cli_returns_json_for_checked_in_target(self):
        ...
        with contextlib.redirect_stdout(stdout):
            exit_code = (
                fixed_point_substitution_representability_proof_target
                .run_fixed_point_substitution_representability_proof_target_cli(
                    [
                        "--target",
                        str(TARGET),
                        "--willard-map",
