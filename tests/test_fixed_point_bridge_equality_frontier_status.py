...
STATUS = Path("claims/fixed_point_bridge_equality_frontier_status.json")
...
class FixedPointBridgeEqualityFrontierStatusTests(unittest.TestCase):
    def setUp(self):
        self.status = load_fixed_point_bridge_equality_frontier_status(STATUS)

    def test_checked_in_manifest_names_bridge_equality_frontier(self):
        self.assertEqual(self.status.schema_version, 1)
        self.assertEqual(
            self.status.status_set_id,
            "as-fixed-point-bridge-equality-frontier-status-v1",
        )
        self.assertEqual(self.status.frontier_status, "blocked")
        self.assertEqual(self.status.frontier_blocked_by, "bridge-equality-proof")
        self.assertEqual(self.status.fixed_point_construction_cases_path, str(CONSTRUCTION_CASES))
        self.assertEqual(
    ...
    def test_checked_in_manifest_validates_bridge_equality_frontier_status(self):
    ...
    def test_json_payload_exposes_compact_frontier_status(self):
        ...
        payload = (
            fixed_point_bridge_equality_frontier_status
        ...
        self.assertEqual(
            payload["support_facts"]["bridge_equality_alignment"][
                "bridge_equation_code_length"
            ],
            4815,
        )
        self.assertEqual(
            payload["support_facts"]["bridge_equality_evaluation"][
                "evaluation_output_code_length"
            ],
            296,
        )
        self.assertEqual(
    ...
    def test_text_report_exposes_blocked_bridge_equality_boundary(self):
        ...
        text = (
            fixed_point_bridge_equality_frontier_status
            .format_fixed_point_bridge_equality_frontier_status_report(report)
        )

        self.assertIn("Fixed-point bridge equality frontier status: accepted", text)
        self.assertIn("Frontier status: blocked", text)
        self.assertIn("Blocked by: bridge-equality-proof", text)
    ...
    def test_proof_promotion_status_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            data["frontier_status"] = "bridge-equality-proved"
            path.write_text(json.dumps(data), encoding="utf-8")
            status = load_fixed_point_bridge_equality_frontier_status(path)

            report = validate_fixed_point_bridge_equality_frontier_status(status)

        self.assertFalse(report.accepted)
        self.assertIn("fixed-point-bridge-equality-frontier-status", report.failed_subjects)
        self.assertTrue(
    ...
    def test_missing_non_claim_is_rejected(self):
        ...
        self.assertFalse(report.accepted)
        self.assertIn("fixed-point-bridge-equality-frontier-non-claim", report.failed_subjects)
    ...
    def test_empty_non_claims_are_rejected_by_loader(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(ValueError):
                load_fixed_point_bridge_equality_frontier_status(path)

    def test_stale_dependency_path_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "status.json"
            data = json.loads(STATUS.read_text(encoding="utf-8"))
            data["bridge_equality_alignment_path"] = str(Path(tmp) / "alignment.json")
            path.write_text(json.dumps(data), encoding="utf-8")
            status = load_fixed_point_bridge_equality_frontier_status(path)
        ...
        self.assertFalse(report.accepted)
        self.assertIn("fixed-point-bridge-equality-frontier-dependency", report.failed_subjects)
        self.assertTrue(
            any(
                result.subject == "bridge_equality_alignment_path"
                and not result.accepted
                for result in report.results
            )
        )

    def test_closed_bridge_equality_construction_case_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            case_path = Path(tmp) / "cases.json"
            case_data = json.loads(CONSTRUCTION_CASES.read_text(encoding="utf-8"))
            for case in case_data["cases"]:
                if case["case_kind"] == "bridge-equality-proof":
                    case["status"] = "bridge-equality-proved"
            ...
            status = load_fixed_point_bridge_equality_frontier_status(status_path)

            report = validate_fixed_point_bridge_equality_frontier_status(status)

        self.assertFalse(report.accepted)
        self.assertIn("fixed-point-bridge-equality-frontier-case-status", report.failed_subjects)
        self.assertTrue(
            any("construction case is not open" in result.detail for result in report.results)
        )

    def test_cli_returns_zero_for_checked_in_bridge_equality_frontier_status(self):
        stdout = io.StringIO()

        with contextlib.redirect_stdout(stdout):
            exit_code = (
                fixed_point_bridge_equality_frontier_status
                .run_fixed_point_bridge_equality_frontier_status_cli(
                    ["--status", str(STATUS)]
                )
            )
        ...
        self.assertIn("Fixed-point bridge equality frontier status: accepted", output)

    def test_cli_returns_json_for_checked_in_bridge_equality_frontier_status(self):
        stdout = io.StringIO()
    ...
    def test_module_execution_runs_bridge_equality_frontier_status_validation(self):
        ...
        self.assertIn(
            "Fixed-point bridge equality frontier status: accepted",
            completed.stdout,
        )

    def test_module_execution_runs_json_bridge_equality_frontier_status_validation(self):
        completed = subprocess.run(
            [
                sys.executable,
                "-m",
