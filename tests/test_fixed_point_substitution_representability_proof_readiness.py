...
from autarkic_systems import fixed_point_substitution_representability_proof_readiness
...
class FixedPointSubstitutionRepresentabilityProofReadinessTests(unittest.TestCase):
    def setUp(self):
        self.readiness = (
            load_fixed_point_substitution_representability_proof_readiness(
                READINESS
    ...
    def test_checked_in_manifest_names_substitution_readiness_inputs(self):
        ...
        self.assertEqual(
            self.readiness.substitution_representability_frontier_status_path,
            str(FRONTIER),
        )
        self.assertEqual(
            self.readiness.substitution_representability_certificate_path,
            str(CERTIFICATE),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.readiness.expected_construction_case_status,
            "proof-case-open",
        )
        self.assertEqual(self.readiness.expected_frontier_status, "blocked")
        self.assertEqual(
            self.readiness.expected_frontier_blocker,
            "substitution-representability-proof",
        )
        ...
        self.assertEqual(
            self.readiness.expected_covered_predecessor_case_kinds,
            (
        ...
        self.assertEqual(
            self.readiness.expected_missing_certificate_predecessor_count,
            0,
        )
        self.assertEqual(self.readiness.expected_witness_output_code_length, 296)
    ...
    def test_checked_in_manifest_validates_substitution_proof_readiness(self):
        ...
        self.assertEqual(
            report.readiness_entry.case_kind,
            "substitution-representability-proof",
        )
        self.assertEqual(
    ...
    def test_json_payload_exposes_certificate_ready_proof_open_boundary(self):
        ...
        self.assertEqual(payload["frontier_blocker"], "substitution-representability-proof")
        ...
        self.assertEqual(entry["case_kind"], "substitution-representability-proof")
    ...
    def test_text_report_exposes_readiness_without_promotion(self):
        ...
        self.assertIn(
            "Fixed-point substitution representability proof readiness: accepted",
            text,
        )
        self.assertIn("Readiness status: blocked-certificate-ready-proof-open", text)
        self.assertIn(
            "Frontier: blocked by substitution-representability-proof",
            text,
        )
    ...
    def test_stale_certificate_step_count_is_rejected(self):
        ...
        self.assertIn(
            "fixed-point-substitution-representability-proof-readiness-certificate",
    ...
    def test_missing_frontier_dependency_is_structured_rejection(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "readiness.json"
            data = json.loads(READINESS.read_text(encoding="utf-8"))
            data["substitution_representability_frontier_status_path"] = (
                "claims/missing_substitution_representability_frontier_status.json"
            )
            path.write_text(json.dumps(data), encoding="utf-8")
            readiness = (
                load_fixed_point_substitution_representability_proof_readiness(
        ...
        self.assertFalse(report.accepted)
        self.assertIn(
            "fixed-point-substitution-representability-proof-readiness-dependencies",
            report.failed_subjects,
        )
    ...
    def test_cli_returns_json_for_checked_in_readiness(self):
        ...
        with contextlib.redirect_stdout(stdout):
            exit_code = (
                fixed_point_substitution_representability_proof_readiness
                .run_fixed_point_substitution_representability_proof_readiness_cli(
                    [
                        "--readiness",
                        str(READINESS),
                        "--willard-map",
