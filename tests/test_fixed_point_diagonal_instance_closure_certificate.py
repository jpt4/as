...
from autarkic_systems import fixed_point_diagonal_instance_closure_certificate
...
CANDIDATE = Path("claims/fixed_point_diagonal_instance_candidate_surface.json")
SELECTOR = Path("claims/fixed_point_frontier_selector.json")
WILLARD_MAP = Path("sources/willard_definition_map.json")


class FixedPointDiagonalInstanceClosureCertificateTests(unittest.TestCase):
    def setUp(self):
        self.certificate = load_fixed_point_diagonal_instance_closure_certificate(
            CERTIFICATE,
        )

    def test_checked_in_manifest_names_closure_certificate_inputs(self):
        ...
        self.assertEqual(self.certificate.diagonal_instance_closure_path, str(CLOSURE))
        self.assertEqual(
            self.certificate.diagonal_instance_candidate_surface_path,
            str(CANDIDATE),
        )
        ...
        self.assertEqual(self.certificate.expected_selected_case_kind, "diagonal-instance-closure")
        self.assertEqual(self.certificate.expected_candidate_code_length, 296)
        self.assertEqual(
            REQUIRED_CERTIFICATE_STEP_IDS,
            (
    ...
    def test_checked_in_manifest_validates_closure_certificate(self):
    ...
    def test_json_payload_exposes_checked_certificate_steps(self):
        ...
        payload = (
            fixed_point_diagonal_instance_closure_certificate
        ...
        self.assertEqual(certificate["selected_case_kind"], "diagonal-instance-closure")
        self.assertEqual(
            certificate["certificate_status"],
            "accepted-finite-certificate-not-proof",
        )
        self.assertEqual(certificate["observed_candidate_code_length"], 296)
        self.assertTrue(certificate["observed_selector_accepts_root"])
        self.assertTrue(certificate["observed_closure_accepted"])
        self.assertTrue(certificate["observed_candidate_surface_accepted"])
        self.assertTrue(certificate["observed_candidate_matches_closure"])
    ...
    def test_text_report_exposes_non_promotional_certificate_boundary(self):
        ...
        text = (
            fixed_point_diagonal_instance_closure_certificate
            .format_fixed_point_diagonal_instance_closure_certificate_report(report)
        )

        self.assertIn("Fixed-point diagonal instance closure certificate: accepted", text)
        ...
        self.assertIn("match-candidate-to-closure: accepted", text)
    ...
    def test_stale_certificate_step_ids_are_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            certificate = load_fixed_point_diagonal_instance_closure_certificate(path)
        ...
        self.assertIn(
            "fixed-point-diagonal-instance-closure-certificate-steps",
    ...
    def test_cli_returns_json_for_checked_in_certificate(self):
        ...
        with contextlib.redirect_stdout(stdout):
            exit_code = (
                fixed_point_diagonal_instance_closure_certificate
                .run_fixed_point_diagonal_instance_closure_certificate_cli(
                    [
                        "--certificate",
                        str(CERTIFICATE),
                        "--willard-map",
