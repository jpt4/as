...
from autarkic_systems import fixed_point_substitution_graph_correctness_proof_readiness
...
FRONTIER = Path("claims/substitution_graph_correctness_frontier_status.json")
CERTIFICATE = Path("claims/fixed_point_substitution_graph_correctness_certificate.json")
WILLARD_MAP = Path("sources/willard_definition_map.json")


class FixedPointSubstitutionGraphCorrectnessProofReadinessTests(unittest.TestCase):
    def setUp(self):
        self.readiness = (
            load_fixed_point_substitution_graph_correctness_proof_readiness(
                READINESS
    ...
    def test_checked_in_manifest_names_graph_correctness_readiness_inputs(self):
        ...
        self.assertEqual(
            self.readiness.substitution_graph_correctness_frontier_status_path,
            str(FRONTIER),
        )
        self.assertEqual(
            self.readiness.substitution_graph_correctness_certificate_path,
            str(CERTIFICATE),
        )
        self.assertEqual(
        ...
        self.assertEqual(self.readiness.expected_correctness_case_count, 5)
        self.assertEqual(self.readiness.expected_finite_dependency_count, 5)
    ...
    def test_checked_in_manifest_validates_graph_correctness_proof_readiness(self):
    ...
    def test_json_payload_exposes_certificate_ready_proof_open_boundary(self):
        ...
        self.assertEqual(payload["frontier_blocker"], "substitution-graph-correctness")
        ...
        self.assertEqual(entry["case_kind"], "substitution-graph-correctness-proof")
    ...
    def test_text_report_exposes_readiness_without_promotion(self):
        ...
        self.assertIn(
            "Fixed-point substitution graph correctness proof readiness: accepted",
            text,
        )
        self.assertIn("Readiness status: blocked-certificate-ready-proof-open", text)
        self.assertIn(
            "Frontier: blocked by substitution-graph-correctness",
            text,
        )
    ...
    def test_stale_certificate_step_count_is_rejected(self):
        ...
        self.assertIn(
            "fixed-point-substitution-graph-correctness-proof-readiness-certificate",
    ...
    def test_missing_frontier_dependency_is_structured_rejection(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "readiness.json"
            data = json.loads(READINESS.read_text(encoding="utf-8"))
            data["substitution_graph_correctness_frontier_status_path"] = (
                "claims/missing_substitution_graph_correctness_frontier_status.json"
            )
            path.write_text(json.dumps(data), encoding="utf-8")
            readiness = (
                load_fixed_point_substitution_graph_correctness_proof_readiness(
        ...
        self.assertFalse(report.accepted)
        self.assertIn(
            "fixed-point-substitution-graph-correctness-proof-readiness-dependencies",
            report.failed_subjects,
        )
    ...
    def test_cli_returns_json_for_checked_in_readiness(self):
        ...
        with contextlib.redirect_stdout(stdout):
            exit_code = (
                fixed_point_substitution_graph_correctness_proof_readiness
                .run_fixed_point_substitution_graph_correctness_proof_readiness_cli(
                    [
                        "--readiness",
                        str(READINESS),
                        "--willard-map",
