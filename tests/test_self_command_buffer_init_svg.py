...
from autarkic_systems.schematic_svg import (
    SELF_COMMAND_BUFFER_INIT_SVG_ARTIFACT,
    SVG_NAMESPACE,
...
TRACE_ARTIFACT = Path("schematics/self_command_buffer_init_trace.json")


class SelfCommandBufferInitSvgTests(unittest.TestCase):
    def setUp(self):
        self.trace = load_schematic_trace(TRACE_ARTIFACT)
        self.svg_text = render_schematic_svg(self.trace)

    def test_svg_is_nonblank_xml_with_command_buffer_trace_metadata(self):
    ...
    def test_svg_labels_command_buffer_ports_from_trace(self):
        root = ET.fromstring(self.svg_text)
    ...
    def test_svg_records_command_buffer_dispatch_details_and_flow(self):
        ...
        self.assertIn("status: stem-command-buffer-self-processed", self.svg_text)
        ...
        self.assertIn("buffer before: [0, 0, 1, 0]", self.svg_text)
    ...
    def test_svg_exposes_command_buffer_interpretive_layer_ids(self):
        root = ET.fromstring(self.svg_text)
    ...
    def test_committed_command_buffer_svg_matches_renderer_output(self):
        committed = SELF_COMMAND_BUFFER_INIT_SVG_ARTIFACT.read_text(encoding="utf-8")

        self.assertEqual(committed, self.svg_text)

    def test_command_buffer_svg_validator_accepts_committed_svg(self):
        results = validate_schematic_svg(
            self.trace,
            svg_text=SELF_COMMAND_BUFFER_INIT_SVG_ARTIFACT.read_text(encoding="utf-8"),
        )

        self.assertTrue(results)
        self.assertTrue(all(result.accepted for result in results), results)

    def test_command_buffer_svg_validator_rejects_drifted_buffer(self):
        drifted = self.svg_text.replace(
            "buffer before: [0, 0, 1, 0]",
            "buffer before: [0, 0, 1, 1]",
        )
