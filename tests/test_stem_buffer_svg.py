...
TRACE_ARTIFACT = Path("schematics/stem_buffer_accumulation_trace.json")


class StemBufferSvgTests(unittest.TestCase):
    def setUp(self):
        self.trace = load_schematic_trace(TRACE_ARTIFACT)
        self.svg_text = render_schematic_svg(self.trace)

    def test_svg_is_nonblank_xml_with_stem_buffer_trace_metadata(self):
    ...
    def test_svg_labels_stem_buffer_ports_from_trace(self):
        root = ET.fromstring(self.svg_text)
    ...
    def test_svg_records_stem_buffer_accumulation_details_and_flow(self):
    ...
    def test_svg_exposes_stem_buffer_interpretive_layer_ids(self):
        root = ET.fromstring(self.svg_text)
    ...
    def test_committed_stem_buffer_svg_matches_renderer_output(self):
        committed = STEM_BUFFER_SVG_ARTIFACT.read_text(encoding="utf-8")

        self.assertEqual(committed, self.svg_text)

    def test_stem_buffer_svg_validator_accepts_committed_svg(self):
        results = validate_schematic_svg(
            self.trace,
            svg_text=STEM_BUFFER_SVG_ARTIFACT.read_text(encoding="utf-8"),
        )

        self.assertTrue(results)
        self.assertTrue(all(result.accepted for result in results), results)

    def test_stem_buffer_svg_validator_rejects_drifted_buffer(self):
        drifted = self.svg_text.replace("buffer after: [0, 1]", "buffer after: [0, 0]")
