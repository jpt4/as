...
BUNDLE = Path("evidence/recipient_non_init_command_rejection_bundle.json")
...
class RecipientNonInitEvidenceBundleTests(unittest.TestCase):
    def setUp(self):
        self.bundle = load_transition_evidence_bundle(BUNDLE)

    def test_bundle_names_the_existing_recipient_non_init_rejection(self):
    ...
    def test_bundle_records_rejection_artifact_paths(self):
        ...
        self.assertEqual(
            ...
            Path("schematics/recipient_non_init_command_rejection_trace.json"),
        )
        self.assertEqual(
            self.bundle.schematic_svg_path,
            Path("schematics/recipient_non_init_command_rejection_trace.svg"),
        )
        self.assertEqual(
    ...
    def test_bundle_validates_rejection_claim_proof_trace_svg_and_statuses(self):
    ...
    def test_registry_includes_init_and_non_init_bundles(self):
    ...
    def test_drifted_rejection_status_is_rejected(self):
        drifted = replace(self.bundle, expected_status="recipient-init-command-message-processed")
