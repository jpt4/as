...
from autarkic_systems import fixed_point_diagonal_instance_closure_proof_target
...
TARGET = Path("claims/fixed_point_diagonal_instance_closure_proof_target.json")
...
class FixedPointDiagonalInstanceClosureProofTargetTests(unittest.TestCase):
    def setUp(self):
        self.target = load_fixed_point_diagonal_instance_closure_proof_target(
            TARGET,
        )
    ...
    def test_checked_in_manifest_names_proof_target_inputs(self):
        ...
        self.assertEqual(
            ...
            "as-fixed-point-diagonal-instance-closure-proof-target-v1",
        )
        self.assertEqual(self.target.diagonal_instance_closure_certificate_path, str(CERTIFICATE))
        self.assertEqual(self.target.diagonal_instance_closure_readiness_path, str(READINESS))
        self.assertEqual(self.target.selected_root_readiness_coverage_path, str(COVERAGE))
        self.assertEqual(
            self.target.expected_case_id,
            "AS-FIXED-POINT-CONSTRUCTION-DIAGONAL-INSTANCE-CLOSURE",
        )
        self.assertEqual(self.target.expected_case_kind, "diagonal-instance-closure")
        self.assertEqual(self.target.expected_proof_target_status, "blocked-proof-closure-targeted")
        self.assertEqual(self.target.expected_readiness_status, "blocked-certificate-ready-proof-open")
        self.assertEqual(self.target.expected_certificate_count, 1)
        self.assertEqual(self.target.expected_certificate_step_count, 7)
        self.assertEqual(self.target.expected_missing_proof_artifacts, REQUIRED_MISSING_PROOF_ARTIFACTS)
    ...
    def test_json_payload_exposes_blocked_proof_target(self):
        ...
        self.assertEqual(payload["case_kind"], "diagonal-instance-closure")
        self.assertEqual(payload["proof_target_status"], "blocked-proof-closure-targeted")
        self.assertEqual(payload["readiness_status"], "blocked-certificate-ready-proof-open")
    ...
    def test_text_report_exposes_target_without_promotion(self):
        ...
        text = (
            fixed_point_diagonal_instance_closure_proof_target
            .format_fixed_point_diagonal_instance_closure_proof_target_report(report)
        )

        self.assertIn("Fixed-point diagonal instance closure proof target: accepted", text)
        self.assertIn("Proof target status: blocked-proof-closure-targeted", text)
        self.assertIn("Certificate steps: 7", text)
        self.assertIn("Missing proof artifacts: formal diagonal-instance closure derivation", text)
    ...
    def test_stale_missing_proof_artifacts_are_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            target = load_fixed_point_diagonal_instance_closure_proof_target(path)
    ...
    def test_missing_dependency_is_structured_rejection(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "target.json"
            data = json.loads(TARGET.read_text(encoding="utf-8"))
            data["diagonal_instance_closure_readiness_path"] = (
                "claims/missing_diagonal_instance_closure_readiness.json"
            )
            path.write_text(json.dumps(data), encoding="utf-8")
            target = load_fixed_point_diagonal_instance_closure_proof_target(path)
        ...
        self.assertFalse(report.accepted)
        self.assertIn(
            "fixed-point-diagonal-instance-closure-proof-target-dependencies",
            report.failed_subjects,
        )
    ...
    def test_cli_returns_json_for_checked_in_target(self):
        ...
        with contextlib.redirect_stdout(stdout):
            exit_code = (
                fixed_point_diagonal_instance_closure_proof_target
                .run_fixed_point_diagonal_instance_closure_proof_target_cli(
                    [
                        "--target",
                        str(TARGET),
                        "--willard-map",
