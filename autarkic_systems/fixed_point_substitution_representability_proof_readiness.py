"""Proof-readiness handoff for substitution representability.

ADR-0313 composes the compact substitution-representability frontier status
with the finite certificate support for that fixed-point construction case.
The resulting surface records that the case is certificate-ready while
preserving that the proof case remains open. It is not a substitution
...
REQUIRED_READINESS_ID = (
    "as-fixed-point-substitution-representability-proof-readiness-v1"
)
...
class FixedPointSubstitutionRepresentabilityProofReadinessManifest:
    """Loaded manifest for the substitution proof-readiness handoff."""
...
@dataclass(frozen=True)
class FixedPointSubstitutionRepresentabilityProofReadinessEntry:
    """Derived readiness state for substitution representability."""
...
@dataclass(frozen=True)
class FixedPointSubstitutionRepresentabilityProofReadinessValidation:
    """One validation result for substitution proof readiness."""
...
@dataclass(frozen=True)
class FixedPointSubstitutionRepresentabilityProofReadinessReport:
    """Validation report for substitution-representability proof readiness."""

    manifest: FixedPointSubstitutionRepresentabilityProofReadinessManifest
    ...
    readiness_entry: FixedPointSubstitutionRepresentabilityProofReadinessEntry
    results: tuple[
        FixedPointSubstitutionRepresentabilityProofReadinessValidation,
        ...,
    ]
...
def load_fixed_point_substitution_representability_proof_readiness(
    path: Path | str = DEFAULT_READINESS,
) -> FixedPointSubstitutionRepresentabilityProofReadinessManifest:
    """Load the substitution-representability proof-readiness manifest."""

    readiness_path = Path(path)
    data = json.loads(readiness_path.read_text(encoding="utf-8"))
    return FixedPointSubstitutionRepresentabilityProofReadinessManifest(
        path=readiness_path,
...
def validate_fixed_point_substitution_representability_proof_readiness(
    manifest: FixedPointSubstitutionRepresentabilityProofReadinessManifest,
    willard_map_path: Path | str = DEFAULT_WILLARD_MAP,
) -> FixedPointSubstitutionRepresentabilityProofReadinessReport:
    """Validate certificate-ready/proof-open readiness for substitution."""

    checked_willard_map_path = Path(willard_map_path)
    paths = _manifest_paths(manifest)
    results: list[FixedPointSubstitutionRepresentabilityProofReadinessValidation] = [
    ...
    return FixedPointSubstitutionRepresentabilityProofReadinessReport(
        manifest=manifest,
...
def fixed_point_substitution_representability_proof_readiness_payload(
    report: FixedPointSubstitutionRepresentabilityProofReadinessReport,
) -> dict[str, Any]:
...
def format_fixed_point_substitution_representability_proof_readiness_report(
    report: FixedPointSubstitutionRepresentabilityProofReadinessReport,
) -> str:
    ...
    lines = [
        f"Fixed-point substitution representability proof readiness: {status}",
...
def run_fixed_point_substitution_representability_proof_readiness_cli(
    argv: list[str] | None = None,
) -> int:
    """Run substitution-representability proof-readiness validation."""

    parser = argparse.ArgumentParser(
        prog=(
            "python -m autarkic_systems."
            "fixed_point_substitution_representability_proof_readiness"
        ),
        description=(
            "Validate certificate-ready/proof-open readiness for the AS "
            "fixed-point substitution-representability proof case."
        ),
    )
    parser.add_argument(
        "--readiness",
        default=str(DEFAULT_READINESS),
        help="Path to the substitution-representability readiness manifest.",
    )
    parser.add_argument(
        "--willard-map",
    ...
    report = validate_fixed_point_substitution_representability_proof_readiness(
        manifest,
        args.willard_map,
    )
    if args.format == "json":
        print(
            json.dumps(
                fixed_point_substitution_representability_proof_readiness_payload(
                    report
                ),
                sort_keys=True,
            )
        ...
        print(
            format_fixed_point_substitution_representability_proof_readiness_report(
                report
            )
        )
...
def _load_frontier(path: Path) -> Any:
    ...
    try:
        ...
        return _DependencyFailure(
            accepted=False,
            failed_subjects=(
                "fixed-point-substitution-representability-frontier-status-load",
            ),
        )


def _load_certificate(path: Path, willard_map_path: Path) -> Any:
    """Load and validate the substitution-representability certificate support."""
...
def _validate_manifest(
    manifest: FixedPointSubstitutionRepresentabilityProofReadinessManifest,
) -> list[FixedPointSubstitutionRepresentabilityProofReadinessValidation]:
...
def _derive_readiness_entry(
    ...
    certificate_report: Any,
    certificate: Any,
) -> FixedPointSubstitutionRepresentabilityProofReadinessEntry:
    ...
    return FixedPointSubstitutionRepresentabilityProofReadinessEntry(
        case_id=str(getattr(frontier_report, "construction_case_id", "")),
        case_kind=str(getattr(frontier_report, "construction_case_kind", "")),
        construction_case_status=str(
            getattr(frontier_report, "construction_case_status", "")
        ),
...
def _validate_readiness(
    ...
    frontier_report: Any,
    certificate_report: Any,
    entry: FixedPointSubstitutionRepresentabilityProofReadinessEntry,
...
def _accepted(
    ...
    return FixedPointSubstitutionRepresentabilityProofReadinessValidation(
        subject=subject,
        accepted=True,
        detail=detail,
    )
...
def _failed_subject_for_result(subject: str) -> str:
    ...
    if subject in {
        ...
        return "fixed-point-substitution-representability-proof-readiness-dependencies"
    if subject in {"certificate_count", "certificate_step_count"}:
        return "fixed-point-substitution-representability-proof-readiness-certificate"
    if subject in {"non_claims", "non_claim_promotion_boundary"}:
        return "fixed-point-substitution-representability-proof-readiness-non-claim"
    if subject == "proof_boundary":
        return "fixed-point-substitution-representability-proof-readiness-boundary"
    if subject in {
        "readiness_case_id",
        "readiness_case_kind",
        "readiness_status",
        ...
        return "fixed-point-substitution-representability-proof-readiness"
    if subject.startswith("expected_") or subject in {"readiness_id", "schema_version"}:
        return "fixed-point-substitution-representability-proof-readiness-manifest"
    return (
...
def main() -> int:
    ...
    return run_fixed_point_substitution_representability_proof_readiness_cli()
