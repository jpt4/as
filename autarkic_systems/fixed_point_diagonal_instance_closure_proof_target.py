"""Blocked proof target for diagonal-instance closure.

ADR-0319 names the current proof-closure gate for the selected
``diagonal-instance-closure`` root obligation. The target ties accepted finite
certificate support, proof-readiness, and selected-root coverage to explicit
missing proof artifacts. It deliberately remains blocked and does not promote
the construction case from ``proof-case-open``.
"""
...
REQUIRED_TARGET_ID = "as-fixed-point-diagonal-instance-closure-proof-target-v1"
...
EXPECTED_PATHS = {
    ...
    "diagonal_instance_closure_readiness_path": (
...
class FixedPointDiagonalInstanceClosureProofTargetManifest:
    """Loaded manifest for the diagonal-instance closure proof target."""
    ...
    diagonal_instance_closure_readiness_path: str
...
@dataclass(frozen=True)
class FixedPointDiagonalInstanceClosureProofTargetValidation:
    """One validation result for the diagonal-instance closure proof target."""
...
@dataclass(frozen=True)
class FixedPointDiagonalInstanceClosureProofTargetReport:
    """Validation report for the diagonal-instance closure proof target."""

    manifest: FixedPointDiagonalInstanceClosureProofTargetManifest
    diagonal_instance_closure_certificate_path: Path
    diagonal_instance_closure_readiness_path: Path
    ...
    results: tuple[FixedPointDiagonalInstanceClosureProofTargetValidation, ...]
...
def load_fixed_point_diagonal_instance_closure_proof_target(
    path: Path | str = DEFAULT_TARGET,
) -> FixedPointDiagonalInstanceClosureProofTargetManifest:
    """Load the diagonal-instance closure proof-target manifest."""

    target_path = Path(path)
    data = json.loads(target_path.read_text(encoding="utf-8"))
    return FixedPointDiagonalInstanceClosureProofTargetManifest(
        path=target_path,
        ...
        diagonal_instance_closure_readiness_path=_required_text(
            data,
            "diagonal_instance_closure_readiness_path",
        ),
...
def validate_fixed_point_diagonal_instance_closure_proof_target(
    manifest: FixedPointDiagonalInstanceClosureProofTargetManifest,
    willard_map_path: Path | str = DEFAULT_WILLARD_MAP,
) -> FixedPointDiagonalInstanceClosureProofTargetReport:
    ...
    readiness_path = _resolve_path(
        manifest.path,
        manifest.diagonal_instance_closure_readiness_path,
    )
    coverage_path = _resolve_path(
        manifest.path,
        manifest.selected_root_readiness_coverage_path,
    )
    results: list[FixedPointDiagonalInstanceClosureProofTargetValidation] = [
    ...
    return FixedPointDiagonalInstanceClosureProofTargetReport(
        manifest=manifest,
        diagonal_instance_closure_certificate_path=certificate_path,
        diagonal_instance_closure_readiness_path=readiness_path,
...
def fixed_point_diagonal_instance_closure_proof_target_payload(
    report: FixedPointDiagonalInstanceClosureProofTargetReport,
) -> dict[str, Any]:
    ...
    return {
        ...
        "diagonal_instance_closure_readiness_path": str(
            report.diagonal_instance_closure_readiness_path
        ),
        "selected_root_readiness_coverage_path": str(
...
def format_fixed_point_diagonal_instance_closure_proof_target_report(
    report: FixedPointDiagonalInstanceClosureProofTargetReport,
) -> str:
    ...
    lines = [
        f"Fixed-point diagonal instance closure proof target: {status}",
...
def run_fixed_point_diagonal_instance_closure_proof_target_cli(
    argv: list[str] | None = None,
) -> int:
    """Run diagonal-instance closure proof-target validation."""

    parser = argparse.ArgumentParser(
        prog=(
            "python -m autarkic_systems."
            "fixed_point_diagonal_instance_closure_proof_target"
        ),
        description=(
    ...
    parser.add_argument(
        ...
        help="Path to the diagonal-instance closure proof-target manifest.",
    )
    parser.add_argument(
        "--willard-map",
    ...
    manifest = load_fixed_point_diagonal_instance_closure_proof_target(args.target)
    report = validate_fixed_point_diagonal_instance_closure_proof_target(
        manifest,
        args.willard_map,
    )
    if args.format == "json":
        print(
            json.dumps(
                fixed_point_diagonal_instance_closure_proof_target_payload(report),
                sort_keys=True,
            )
        )
    else:
        print(format_fixed_point_diagonal_instance_closure_proof_target_report(report))
...
def _load_certificate(path: Path, willard_map_path: Path) -> Any:
    """Load and validate diagonal-instance certificate support."""
    ...
    try:
        ...
        return _DependencyFailure(
            accepted=False,
            failed_subjects=("fixed-point-diagonal-instance-certificate-load",),
        )


def _load_readiness(path: Path, willard_map_path: Path) -> Any:
    """Load and validate diagonal-instance proof-readiness."""

    try:
        manifest = load_fixed_point_diagonal_instance_closure_proof_readiness(path)
        return validate_fixed_point_diagonal_instance_closure_proof_readiness(
            manifest,
            willard_map_path,
        )
    except (OSError, ValueError, TypeError, AttributeError, json.JSONDecodeError):
        return _DependencyFailure(
            accepted=False,
            failed_subjects=("fixed-point-diagonal-instance-readiness-load",),
        )
...
def _validate_manifest(
    manifest: FixedPointDiagonalInstanceClosureProofTargetManifest,
) -> list[FixedPointDiagonalInstanceClosureProofTargetValidation]:
...
def _validate_target(
    ...
    return [
        _check("target.case_id", case_id == manifest.expected_case_id, "case id matches", "case id mismatch"),
        _check("target.case_kind", case_kind == manifest.expected_case_kind, "case kind matches", "case kind mismatch"),
        _check("target.proof_target_status", proof_target_status == manifest.expected_proof_target_status, "proof target status matches", "proof target status mismatch"),
        _check("target.readiness_status", readiness_status == manifest.expected_readiness_status, "readiness status matches", "readiness status mismatch"),
        _check("target.certificate_count", certificate_count == manifest.expected_certificate_count, "certificate count matches", "certificate count mismatch"),
        _check("target.certificate_step_count", certificate_step_count == manifest.expected_certificate_step_count, "certificate step count matches", "certificate step count mismatch"),
        _check("target.missing_proof_artifacts", missing_proof_artifacts == manifest.expected_missing_proof_artifacts, "missing proof artifacts match", "missing proof artifact mismatch"),
        _check("target.missing_proof_artifact_count", len(missing_proof_artifacts) == manifest.expected_missing_proof_artifact_count, "missing proof artifact count matches", "missing proof artifact count mismatch"),
        _check("target.proof_closure_blocked", len(missing_proof_artifacts) > 0, "proof closure remains blocked", "proof closure unexpectedly ready"),
        _check("proof_boundary", proof_boundary_preserved, "proof boundary preserved", "proof boundary not preserved"),
    ]
...
) -> FixedPointDiagonalInstanceClosureProofTargetValidation:
    """Build a validation result."""

    return FixedPointDiagonalInstanceClosureProofTargetValidation(
        subject=subject,
        accepted=accepted,
...
def _failed_subject_for_result(subject: str) -> str:
    ...
    if subject.startswith("certificate.") or subject.startswith("readiness.") or subject.startswith("selected_root_coverage."):
        return "fixed-point-diagonal-instance-closure-proof-target-dependencies"
    if subject.startswith("target.missing_proof_artifact") or subject.startswith(
        "expected_missing_proof_artifact"
    ):
        return "fixed-point-diagonal-instance-closure-proof-target-artifacts"
    if subject.startswith("target."):
        return "fixed-point-diagonal-instance-closure-proof-target-status"
    if subject.startswith("proof_boundary") or subject.startswith("non_claim_promotion_boundary"):
        return "fixed-point-diagonal-instance-closure-proof-target-proof-boundary"
    return "fixed-point-diagonal-instance-closure-proof-target-manifest"
...
def main() -> int:
    ...
    return run_fixed_point_diagonal_instance_closure_proof_target_cli()
